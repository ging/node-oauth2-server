...
debug("CODIGO query"+request.query.code)
debug("CODIGO body"+request.body.code)
...
AuthorizationCodeGrantType.prototype.saveToken = function(user, client, authorizationCode, scope, request) {
  ...
  var fns = [
    ...
    this.generateIDToken(user, client, request, authorizationCode)
  ];

  return Promise.all(fns)
    .bind(this)
    .spread(function(scope, accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt, idToken) {
      var token = {
