...
function TokenModel(data, options) {
  ...
  if (!Object.keys(data).some(elem => (['user', 'iot', 'pep_proxy'].includes(elem) && data.elem !== null)) && (data.client.grants.indexOf('client_credentials') == -1) ) {
  ...
  this.pep_proxy = (data.pep_proxy) ? data.pep_proxy : null;
