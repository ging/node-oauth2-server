...
var debug = require('debug')('oauth2-server: token-handler')
...
/**
 * Get ID Token from the model
 */
 /*TokenHandler.prototype.getIDToken = function(request, client) {
   debug("=====TokenHandler: getIDToken=====")
   //debug("********REQUEST*******************"+ request)
   //debug("************************CLIENT*****"+ client)

   return promisify(this.model.getIDToken, 2).call(this.model, client, identity)
...
TokenHandler.prototype.getClientCredentials = function(request) {
  debug("=====TokenHandler: getClientCredentials=====")
...
TokenHandler.prototype.handleGrantType = function(request, client) {
  debug("=====TokenHandler: handleGrantType=====")
  //var grantType = request.query.grant_type;
  var grantType = request.body.grant_type;
  debug(request.body)
  debug(request.query)
  debug(grantType)
  debug("*********SCOPE*******"+request.scope)
  ...
  var idToken = 'EJEMPLO ID_TOKEN';
  var Type = this.grantTypes[grantType];
  //var code = this.model.getAuthorizationCode(request, client);
  debug("!!!!!!!!!!!2222222222"+this.model);
  var options = {
...
TokenHandler.prototype.getAccessTokenLifetime = function(client) {
  debug("=====TokenHandler: GetAccessTokenLifeTime=====")
...
TokenHandler.prototype.getRefreshTokenLifetime = function(client) {
  debug("=====TokenHandler: getRefreshTokenLifeTime=====")
...
TokenHandler.prototype.getTokenType = function(model) {
  debug("=====TokenHandler: getTokenType=====")
  return new BearerTokenType(model.accessToken, model.idToken, model.accessTokenLifetime, model.refreshToken, model.scope, model.customAttributes);
};
...
TokenHandler.prototype.updateSuccessResponse = function(response, tokenType) {
  debug("=====TokenHandler: updateSuccessResponse=====")
...
TokenHandler.prototype.updateErrorResponse = function(response, error) {
  debug("=====TokenHandler: UpdateErrorResponse=====")
  response.body = {
    error: error.name,
...
TokenHandler.prototype.isClientAuthenticationRequired = function(grantType) {
  debug("=====TokenHandler: isClientAuthenticationRequired =====")
